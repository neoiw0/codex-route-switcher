...
3. 输入 `1` 选 CC - Fox（可选），或输入 `2` 选 CC - OpenCode Go（推荐 / 默认）；输入 `3` 可测试连接，输入 `4` 可修复历史聊天供应商（只改供应商名字，聊天模型不变）。Enter `1` for Fox (optional), `2` for OpenCode Go (recommended / default), `3` to test the connection, or `4` to repair old chat providers (name only, chat models kept).
4. 选择模型，再选择思考强度。切换后所有旧聊天也会自动变成你选的模型。Pick a model and a reasoning effort. After the switch, all old chats automatically use the model you picked.
...
7. 以后想换模型或强度，重新双击运行即可，旧聊天会自动跟着换。Run again to change model or effort; old chats follow automatically.
...
- **旧聊天记录打不开 / 消失（之前用 Fox 等供应商）**：运行切换器输入 `4`，脚本会备份状态数据库，把数据库和所有旧会话的供应商改写成 CC（只改名字，聊天模型不变），然后完全退出重开 Codex 即可。*Old chats fail to open / disappear*: run the switcher and enter `4`; it backs up the state database and rewrites only the provider to CC (chat models are kept), then fully quit and reopen Codex.
- **旧聊天仍显示旧模型（如 luna）**：正常切换一次即可——每次切换时脚本都会自动把所有旧聊天同步成你选的模型（默认 DeepSeek 系列），不用单独操作。*Old chats still show a stale model (e.g. luna)*: just run a normal switch; every switch automatically syncs all old chats to the model you picked.
