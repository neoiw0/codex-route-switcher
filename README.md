...
- OpenCode Pro 经桥后默认以**纯对话模式**运行（请求不带工具）：桌面版在 Pro 工具轮执行后存在应用级卡死（Codex 桌面版内部问题，工具侧无法修复），去掉工具可保证任何问题都稳定回答。需要 Pro 调用工具（如 apply_patch / 命令）请改用菜单 2 的 DeepSeek 官方 API，或在 `codex-opencode-pro-bridge.py` 启动环境中设置 `CODEX_KEEP_TOOLS=1`（仅建议 CLI 场景）。*OpenCode Pro runs in chat-only mode through the bridge because the desktop app hangs after tool rounds on this route; use DeepSeek official API (option 2) for full tool support.*
...
- **Pro 为什么不能调用工具（apply_patch / 命令）？**：桌面版在 OpenCode Pro 的工具轮执行后会卡死（应用内部问题），桥默认以纯对话模式运行来保证稳定。需要完整工具能力请选菜单 2（DeepSeek 官方 API）。*Why is OpenCode Pro chat-only?* The desktop app hangs after tool rounds on this route, so the bridge strips tools for stability; use the official DeepSeek API for full tool support.
