...
3. 输入 `1` 选 CC - Fox（可选），或输入 `2` 选 CC - OpenCode Go（推荐 / 默认）；输入 `3` 可测试连接，输入 `4` 可修复历史聊天供应商。Enter `1` for Fox (optional), `2` for OpenCode Go (recommended / default), `3` to test the connection, or `4` to repair old chat providers.
