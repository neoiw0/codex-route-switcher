# Codex Route Switcher（四合一版 / All-in-One Edition）
...
A one-click tool that switches Codex Desktop's API route to third-party suppliers (OpenCode Go / DeepSeek official / Fox / SCNet direct), rewrites `~/.codex/config.toml`, and launches Codex.

## 特点 / Features

- 不含任何 API Key：Key 只保存在你自己的 Windows 用户环境变量中（`OPENCODE_API_KEY` / `DEEPSEEK_API_KEY` / `FOX_API_KEY` / `SCNET_API_KEY`），不会写进脚本或旁边的文件。
- 不需要科学上网、不需要看门狗。OpenCode / DeepSeek 官方 / Fox / 超算中心均大陆直连（Cloudflare 线路偶尔抖动，失败重试即可）；OpenCode 的 Pro 由内置本地小桥自动转换格式（只监听本机 `127.0.0.1`，无需手动操作）。
- 中文不再报错：主脚本带 UTF-8 BOM（Windows PowerShell 5.1 在任何区域设置的电脑上都能正确解析中文菜单）；脚本自己发出的所有请求都按 UTF-8 字节发送，JSON 里的中文会被转义成 `\uXXXX` 纯 ASCII 报文，从传输层杜绝编码错乱。Chinese-safe: the script ships with a UTF-8 BOM and every request it sends is UTF-8 encoded with non-ASCII escaped to `\uXXXX`.
...
- 超算中心 SCNet 直连 `https://api.scnet.cn/api/llm/v1`（OpenAI 兼容 chat completions 格式，切换器自动把 Codex 的 wire_api 配置为 chat）。
- 配置是永久的：重启电脑、重开应用都有效。

All four suppliers connect directly - no local proxy or VPN needed. The config is permanent and survives reboots.
...
- 接口格式：OpenAI 兼容的 chat completions（`https://api.scnet.cn/api/llm/v1`）。**注意：新版 Codex（26.818+ / CLI 0.149+）已彻底移除 chat 协议**（官方 [discussion #7782](https://github.com/openai/codex/discussions/7782)），SCNet 直连在新版上暂不可用——切换器写入后会用官方 CLI 自动校验，被拒绝就回滚到你之前的路由，绝不会再出现配置损坏导致的 "Windows setup didn't finish" 卡死。本地桥（responses→chat 转换）已在计划中。
...
Models: the full gateway list is fetched at runtime (37 entries as of 2026-08), with `DeepSeek-V4-Flash-0731-Event` auto-appended even though it is no longer listed (verified still requestable - HTTP 429 rate-limit, not 404). **New Codex builds (26.818+) removed the chat protocol entirely** ([discussion #7782](https://github.com/openai/codex/discussions/7782)); SCNet direct therefore needs a local bridge on new builds - until then the switcher validates every write with the official CLI and rolls back automatically instead of leaving a broken config.
...
- 超算中心 SCNet：在国家超算互联网平台（<https://www.scnet.cn>）注册，在控制台开通模型服务并创建 API Key。
...
- **旧聊天记录打不开 / 消失（之前用 Fox 等供应商）**：运行切换器输入 `6`，脚本会备份状态数据库，把数据库和所有旧会话的供应商改写成 CC（只改名字，聊天模型不变），然后完全退出重开 Codex 即可。*Old chats fail to open / disappear*: run the switcher and enter `6`; it backs up the state database and rewrites only the provider to CC (chat models are kept), then fully quit and reopen Codex.
...
- **Codex 打开就要求登录（新版强制登录）**：运行切换器输入 `7`——用**官方 CLI** 的 `login --with-api-key` 写入 API Key 登录态（auth.json 由官方代码生成、格式与当前版本严格一致；占位 key 不会发给任何服务器；已有官方登录会先自动备份），随后重启 Codex 直接可用。想恢复官方账号登录：运行 `codex logout` 或删除 auth.json 再登录。*Forced login screen*: run option `7`; it performs an official-CLI API-key login with a placeholder key (never sent anywhere; existing logins backed up first). `codex logout` reverts.
- **"Windows setup didn't finish" 反复循环 / 聊天列表空白**：这是新版 Codex（26.818+）拒绝加载不兼容配置的表现——例如旧版切换器给 SCNet 写入的 `wire_api = "chat"` 已被官方移除（[#7782](https://github.com/openai/codex/discussions/7782)）。手动修复：把 `%USERPROFILE%\.codex\config.toml` 里的 `wire_api = "chat"` 改成 `"responses"` 后重启 Codex。最新版切换器每次写入配置后都会用官方 CLI 自动校验，不兼容直接回滚，不会再把应用弄进这个状态。*"Windows setup didn't finish" loop / empty chat list*: the new build refuses configs containing the removed `wire_api = "chat"`; fix manually or let the latest switcher auto-validate and roll back.
...
脚本里没有任何 API Key。你的 Key 只保存在本机 Windows 用户环境变量中（`OPENCODE_API_KEY` / `DEEPSEEK_API_KEY` / `FOX_API_KEY` / `SCNET_API_KEY`），改 Key 只需重新运行并回答 `n` 再输入一次。`switch.log` 只记录运行时间、供应商和所选模型，用于排查问题。
