...
- OpenCode Pro 经桥后默认只禁用两个工具：`shell_command`（实测其工具轮会让桌面版在工具执行后卡死整个应用）和 `request_user_input`（会主动暂停回合，表现为"卡住"）。其余工具（apply_patch、update_plan、get_goal、create_goal、list_mcp_resources、read_mcp_resource、view_image、mcp__node_repl 等）已实测在桌面版上逐项通过、全程无卡死。需要 Pro 跑命令请改用菜单 2 的 DeepSeek 官方 API。*The bridge strips only shell_command (its tool round hangs the desktop app) and request_user_input; all other tools are verified working on OpenCode Pro. Use DeepSeek official API for shell work.*
...
- **Pro 为什么不能跑 shell 命令？**：实测 OpenCode Pro 的 `shell_command` 工具轮会让桌面版在工具执行后卡死整个应用（其它工具全部正常）。桥默认只禁用 `shell_command` 和 `request_user_input`；需要跑命令请选菜单 2（DeepSeek 官方 API）。*Why no shell on OpenCode Pro?* Its shell tool round freezes the desktop app; every other tool works. Use the official DeepSeek API for shell work.
